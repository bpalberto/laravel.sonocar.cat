...
$document.ready( function () {
	...
	*/
	...
	// Subscribe Form
	if ( plugins.subscribeForm.length ) {
		var i, j, k,
			msg = {
				'SF000': 'Successfully subscribed!',
				'SF001': 'Already subscribed.',
				'SF002': 'Successfully deleted from mailing list!',
                                'SF254': 'You are not currently subscribed or mail dir is incorrect!',
				'SF255': 'Aw, snap! Something went wrong.'
			};

			for ( i = 0; i < plugins.subscribeForm.length; i++) {
				var $form = $( plugins.subscribeForm[ i ] );

				$form.attr( 'novalidate', 'novalidate' ).ajaxForm( {
					data: {
						"form-type": $form.attr( "data-form-type" ) || "subscribe",
						"counter": i
					},
					beforeSubmit: function ( arr, $form, options ) {
						var form = $( plugins.subscribeForm[ this.extraData.counter ] ),
						...
						if ( output.hasClass( "snackbars" ) ) {
							output.html( '<p><span class="icon mdi mdi-autorenew mdi-spin icon-xxs"></span><span>Sending...</span></p>' );
					...
					error: function ( result ) {
						...
						var output = $( "#" + $( plugins.subscribeForm[ this.extraData.counter ] ).attr( "data-form-output" ) ),
							form = $( plugins.subscribeForm[ this.extraData.counter ] );
					...
					success: function ( result ) {
						...
						result = result.length === 5 ? result : 'SF255';
						output.text( msg[ result ] );

						if ( result === "SF000" ) {
							output.html( '<p><span class="icon text-middle mdi mdi-check icon-xxs"></span><span>' + msg[ result ] + '</span></p>' );
							output.addClass( "active success" );
						} else if (result === "SF002") {
							output.html( '<p><span class="icon text-middle mdi mdi-check icon-xxs"></span><span>' + msg[ result ] + '</span></p>' );
							output.addClass( "active success" );
						} else {
	...
	if ( plugins.rdMailForm.length ) {
		var i, j, k,
			msg = {
				...
				'MF005': 'Please, define the message! (10 characters +)',
